function play() {
  ...
  if (titanGroup.length !== 0) {
    ...
    //makes the titanhurt image wait 0.5 seconds or 500 miliseconds before the image changes
  ...
  /* changes the playerImage back to standing. playerSprite.y > 600 because 
  playerSprite.y is 600 when eren stands on the ground. 
  The value might be different for your screen.*/
  if (playerSprite.y > 600 && !keyDown(RIGHT_ARROW)) {
...
function start() {
  ...
  fill(0);
  text(
    "This game is set in a world where humanity lives inside cities surrounded by three enormous walls",
    430,
    310
  );
  text(
    "that protect them from the gigantic man-eating humanoids referred to as Titans.",
    430,
    340
  );
  text(
    "The story follows Eren Yeager, who vows to exterminate the Titans and avenge his loved ones.",
    430,
    370
  );
...
function spawnTitans() {
  if (titanGroup.length === 0 && second() == (sec + 5) % 60) {
    titanHealth = 100;
    titanSprite = createSprite(width - 200, 750, 20, 20);
...
function spawnFire() {
  if (frameCount % 60 === 0) {
    ...
    if (rand === 2) {
      var fireBall = createSprite(titanSprite.x - 100, titanSprite.y +100, 20, 10);
